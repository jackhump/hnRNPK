...
all_exons <- read_tsv(here::here("Buratti/all_cassette_metadata_final.tsv"))

conservation <- read_tsv(here::here("Buratti/all_cassette_metadata_phyloP.tsv"))

all_exons$mean_phyloP_score <- conservation$mean_phyloP_score[match(all_exons$clusterID, conservation$clusterID)]

load(here::here("Buratti/DESeq2_results.RData"))
...
ggsave(plot = deseq_plot, filename = here::here("plots/Buratti_hnRNPK_deseq2_volcano.pdf"), width = 5, height = 4)

# write out
write_tsv(deseq_res, file = here::here("Tables/deseq_res.tsv"))
...
ggsave(plot = full_plot, filename = here::here("plots/all_exons_summary_plot.pdf"), width = 4, height = 3)
...
ggsave(plot = inset_plot, width = 3, height = 5, filename = here::here("plots/cryptic_inset_plot.pdf"))
```
...
ggsave(plot = phylop_plot, filename = here::here("plots/PhyloP_conservation_plot.pdf"), width = 4, height = 3 )
```
...
ggsave(plot = phylop_category_plot, filename = here::here("plots/PhyloP_conservation_category_plot.pdf"), width = 6, height = 6 )
```
...
write_tsv(all_exons_out, file = here::here("Tables/all_exons_res.tsv"))
```
...
if(rerun){
  ...
  write_tsv(exon_go$result %>% dplyr::select(-parents), path = here::here("Tables/all_exons_go_terms.tsv"))
  #write_tsv(exon_kegg$result %>% dplyr::select(-parents), path = "Buratti/all_exons_KEGG_terms.tsv")
}else{
  exon_go_res <- read_tsv(here::here("Tables/all_exons_go_terms.tsv"))
...
ggsave(plot = deg_violin_plot, filename = here::here("plots/deg_violin_plot.pdf"), width = 4, height = 5)
...
ggsave(plot = cryptic_skiptic_expression_plot, filename = here::here("plots/deg_cryptic_skiptics.pdf"), width = 5, height = 4)
...
ggsave(plot = width_plot,filename = here::here("plots/all_cassette_width_plot.pdf"), width = 5, height = 5 )
